...
def Product_time(Time_start, Time_end, type=1):
    ...
    Time_s_befor = (datetime.datetime.utcfromtimestamp(Time_s_now) - datetime.timedelta(days=1)).strftime("%Y%m%d%H")
    Time_e_now = time.mktime(time.strptime(Time_e_zu, "%Y-%m-%d %H:%M:%S"))
    Time_e_befor = (datetime.datetime.utcfromtimestamp(Time_e_now) - datetime.timedelta(days=1)).strftime("%Y%m%d%H")
    if type == 1:
